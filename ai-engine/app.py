...
from rag import ingest_chunks, query_similar, get_collection_stats
...
# 🟢 Route: Vector similarity search (query ChromaDB)
@app.post("/api/rag/query", response_model=QueryResponse)
async def search_similar(request: QueryRequest):
    if not request.query.strip():
        raise HTTPException(status_code=400, detail="Query text is required.")

    n_results = min(max(request.n_results or 5, 1), 50)
    results = query_similar(request.query, n_results=n_results)
